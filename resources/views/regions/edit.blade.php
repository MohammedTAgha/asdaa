...
@section('content')
    <div class="container mx-auto py-6">
        <h1 class="text-3xl font-bold mb-6">تعديل منطقة</h1>
            ...
            <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="position" id="position" 
                           ...
                           value="{{ old('position', $region->position) }}" required>
                    @error('position')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <select name="big_region_id" id="big_region_id" 
                        ...
                        @foreach($bigRegions as $bigRegion)
                            <option value="{{ $bigRegion->id }}" 
                                {{ old('big_region_id', $region->big_region_id) == $bigRegion->id ? 'selected' : '' }}>
                                {{ $bigRegion->name }} 
                        ...
                        @foreach($representatives as $representative)
                            <div class="flex items-center">
                                <input type="checkbox" name="representative_ids[]" 
                                       ...
                                       {{ in_array($representative->id, old('representative_ids', $region->representatives->pluck('id')->toArray())) ? 'checked' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="note" id="note" rows="3" 
                              class="form-textarea w-full @error('note') border-red-500 @enderror">{{ old('note', $region->note) }}</textarea>
                    @error('note')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
