...
<body>
    <div class="container mx-auto py-12">
        <h1 class="text-4xl font-bold mb-4">Edit Region</h1>
        <form action="{{ route('regions.update', $region->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="name" class="block text-gray-700">Name:</label>
                <input type="text" name="name" id="name" value="{{ $region->name }}" class="w-full px-4 py-2 border rounded-md" required>
            </div>
            <div class="mb-4">
                <label for="position" class="block text-gray-700">Position:</label>
                <input type="text" name="position" id="position" value="{{ $region->position }}" class="w-full px-4 py-2 border rounded-md" required>
            </div>
            <div class="mb-4">
