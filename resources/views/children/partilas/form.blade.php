    ...
    <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <form action="{{ isset($child) ? route('children.update', $child->id) : route('children.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="citizen_id" class="block text-sm font-medium text-gray-700">Father:</label>
            ...
            <div class="mt-6 flex justify-end">
                <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">{{ isset($child) ? 'Update Child' : 'Add Child' }}</button>
                <a href="{{ route('children.index') }}" class="px-4 py-2 bg-blue-600 text-white rounded-md">Back to List</a>
